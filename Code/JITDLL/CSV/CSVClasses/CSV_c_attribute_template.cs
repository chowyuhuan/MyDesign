...
public partial class CSV_c_attribute_template : CSVBase
{
	#region data
	//<BeginAttribute>
	public int Id;
	...
	private static List<CSV_c_attribute_template> csv_data = new List<CSV_c_attribute_template>();
	...
	private static void InitCSVTable()
	{
		...
		TextAsset ta;
		#if !USE_ASSETBUNDLE
		ta = Resources.Load("Configs/csv/c_csv/c_attribute_template", typeof(TextAsset)) as TextAsset;
		#else
		ta = AssetBundles.AssetBundleManager.Instance.LoadSync<TextAsset>("Configs/csv/c_csv/c_attribute_template", AssetBundles.E_Resource_Type.Normal, true);
		#endif
		...
		{
			CSV_c_attribute_template item 		= new CSV_c_attribute_template();
	...
	public static CSV_c_attribute_template GetData( int index )
	{
		if( IsInited == false )
		{
			InitCSVTable();
    ...
    /// <returns>CSV的一行数据</returns>
    public static CSV_c_attribute_template FindData(int index)
    {
        if (IsInited == false)
        {
            InitCSVTable();
	...
	public static List<CSV_c_attribute_template> FindAll(int index)
    {
        if (IsInited == false)
        {
            InitCSVTable();
    ...
    /// 返回所有数据
    /// </summary>
	public static List<CSV_c_attribute_template> AllData
	{
		get
		{
			if (IsInited == false)
